...
#===Provide descriptive feature names to columns in X===========
names(X) <- c('tBodyAcc-mean()-X', 
              'tBodyAcc-mean()-Y', 
              'tBodyAcc-mean()-Z', 
              'tBodyAcc-std()-X', 
              'tBodyAcc-std()-Y', 
...
#===Drop all but mean and std columns from X======================
m_s_cols <- c(grep('mean', names(X)), grep('std', names(X)))
X <- select(X, all_of(m_s_cols))

#===Putting activity and subject in the data frame X. y is left as a 
#===look-up table for activity names. This leaves all columns of X
#===numeric, which is easier for the grouping and summarizing question
tot <- cbind(y, X)
tot <- cbind(subject, tot)

#===Use meaningful activity names==================================
y$activityname[y$activity == 1] <- 'WALKING'
y$activityname[y$activity == 2] <- 'WALKING_UPSTAIRS'
y$activityname[y$activity == 3] <- 'WALKING_DOWNSTAIRS'
y$activityname[y$activity == 4] <- 'SITTING'
y$activityname[y$activity == 5] <- 'STANDING'
y$activityname[y$activity == 6] <- 'LAYING'
